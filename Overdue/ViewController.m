...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    else if ([segue.destinationViewController isKindOfClass:[DetailedTaskViewController class]]) {
        DetailedTaskViewController *detailedTaskViewController = segue.destinationViewController;
        NSIndexPath *path = sender;
        Task *taskObject = self.taskObjects[path.row];
        detailedTaskViewController.task = taskObject;
...
#pragma mark - DetailedTaskViewControllerDelegate
...
-(NSDictionary *)taskObjectsAsPropertyList:(Task *)taskObject {
    NSDictionary *dictionary = @{TASK_TITLE : taskObject.title, TASK_DETAILS : taskObject.details, TASK_DATE : taskObject.date, TASK_COMPLETION : @(taskObject.isCompleted)};
    return dictionary;
}
...
-(void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    [self performSegueWithIdentifier:@"toDetailedTaskViewControllerSegue" sender:indexPath];
}
